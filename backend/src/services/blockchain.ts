...
const contractABI = [
    "function mintBatch(address to, string memory uri, string memory origin, string memory supplierId, uint256 carbonFootprint) public returns (uint256)",
    ...
    "function batches(uint256 tokenId) public view returns (string origin, uint256 timestamp, string supplierId, uint256 carbonFootprint, int256 temperature, uint256 humidity, bool complianceStatus, string currentHolder)",
...
class BlockchainService {
    ...
    async mintBatch(
        ...
        const tx = await this.contract.mintBatch(to, uri, origin, supplierId, carbonFootprint);
